...
def generate_invitations(template, attendees):
    ...
    if not isinstance(template, str):
        raise TypeError(f"Invalid input: template must be a string, but got {type(template).__name__}.")
    if len(template) == 0:
        raise ValueError("Template is empty, no output files generated.")
    if not isinstance(attendees, list):
        raise TypeError(f"Invalid input: attendees must be a list, but got {type(attendees)}.")
    if not attendees:
        raise ValueError("No data provided, no output files generated.")
    for a in attendees:
        if not isinstance(a, dict):
            raise TypeError(f"Invalid input: each attendee must be a dictionary, but got {type(a).__name__}.")
